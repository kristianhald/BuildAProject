...
namespace BuildAProject.Console.Test.Configuration
{
  ...
  public sealed class ConsoleConfigurationTests
  {
    ...
    #region NuGetPackagePath

    [Test]
    public void NuGetPackagePath_WhenCalledBeforeParse_ThrowsError()
    {
      // Arrange
      var configuration = new ConsoleConfiguration();

      // Act + Assert
      Assert.Throws<Exception>(() => { var notUsed = configuration.NuGetPackagePath; });
    }

    [Test]
    public void NuGetPackagePath_WithShortFormArgument_ThenNuGetPackagePathInConfigurationIsSet()
    {
      // Arrange
      const string nuGetPackagePath = "SomeNuGetPackagePath";
      var expectednuGetPackagePath = Path.GetFullPath(nuGetPackagePath);

      var arguments = new ConsoleConfigurationArgumentsBuilder
      {
        NuGetPackagePathShortFormValue = nuGetPackagePath
      }.Build();
      ...
      // Act
      configuration.Parse(arguments.Split(' '));
      var actualNuGetPackagePath = configuration.NuGetPackagePath;

      // Assert
      Assert.AreEqual(expectednuGetPackagePath, actualNuGetPackagePath);
    }

    [Test]
    public void NuGetPackagePath_WithLongFormArgument_ThenNuGetPackagePathInConfigurationIsSet()
    {
      // Arrange
      const string nuGetPackagePath = "SomeNuGetPackagePath";
      var expectednuGetPackagePath = Path.GetFullPath(nuGetPackagePath);
      ...
      var arguments = new ConsoleConfigurationArgumentsBuilder
      {
        NuGetPackagePathLongFormValue = nuGetPackagePath
      }.Build();
    ...
    [Test]
    public void NuGetPackagePath_WhenArgumentIsMissing_ReturnsFalse()
    {
      // Arrange
      const string arguments = "-l 2";
